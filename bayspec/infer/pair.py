...
class Pair:
    ...
    def _convolve(self):
        ...
        phtflux = [
            flat_phtflux[i:j]
            for (i, j) in zip(self.data.bin_start, self.data.bin_stop, strict=False)
        ]
        ctsrate = [
            np.dot(pf, drm) for (pf, drm) in zip(phtflux, self.data.corr_rsp_drm, strict=False)
        ]
    ...
    def _convolve_f64(self):
        ...
        ctsrate = [
            ...
            for (pf, drm) in zip(phtflux, self.data.corr_rsp_drm, strict=False)
        ]
    ...
    def _re_convolve(self):
        ...
        re_ctsrate = [
            np.dot(pf, drm) for (pf, drm) in zip(phtflux, self.data.corr_rsp_re_drm, strict=False)
        ]
    ...
    def conv_ctsspec(self):
        ...
        return [
            cr / chw
            for (cr, chw) in zip(self.conv_ctsrate, self.data.rsp_chbin_width, strict=False)
        ]
    ...
    def conv_re_ctsspec(self):
        ...
        return [
            cr / chw
            for (cr, chw) in zip(self.conv_re_ctsrate, self.data.rsp_re_chbin_width, strict=False)
        ]
    ...
    def phtspec_at_rsp(self):
        ...
        return [
            self.model.phtspec(E, T)
            for (E, T) in zip(self.data.rsp_chbin_mean, self.data.rsp_chbin_tarr, strict=False)
        ]
    ...
    def re_phtspec_at_rsp(self):
        ...
        return [
            self.model.phtspec(E, T)
            for (E, T) in zip(
                self.data.rsp_re_chbin_mean, self.data.rsp_re_chbin_tarr, strict=False
            )
        ]
    ...
    def rate_to_flux(self):
        ...
        ergflux = [
            np.sum([self.model.ergflux(emin, emax, 1000, time) for emin, emax in notc])
            for notc, time in zip(self.data.notcs, self.data.times, strict=False)
        ]
    ...
    def deconv_phtspec(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.cts_to_pht, self.data.net_ctsspec, strict=False)
        ]
    ...
    def deconv_re_phtspec(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.re_cts_to_pht, self.data.net_re_ctsspec, strict=False)
        ]
    ...
    def deconv_phtspec_error(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.cts_to_pht, self.data.net_ctsspec_error, strict=False)
        ]
    ...
    def deconv_re_phtspec_error(self):
        ...
        return [
            ...
            for (factor, cts) in zip(
                self.re_cts_to_pht, self.data.net_re_ctsspec_error, strict=False
            )
        ]
    ...
    def deconv_flxspec(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.cts_to_flx, self.data.net_ctsspec, strict=False)
        ]
    ...
    def deconv_re_flxspec(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.re_cts_to_flx, self.data.net_re_ctsspec, strict=False)
        ]
    ...
    def deconv_flxspec_error(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.cts_to_flx, self.data.net_ctsspec_error, strict=False)
        ]
    ...
    def deconv_re_flxspec_error(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(
                self.re_cts_to_flx, self.data.net_re_ctsspec_error, strict=False
            )
        ]
    ...
    def deconv_ergspec(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.cts_to_erg, self.data.net_ctsspec, strict=False)
        ]
    ...
    def deconv_re_ergspec(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.re_cts_to_erg, self.data.net_re_ctsspec, strict=False)
        ]
    ...
    def deconv_ergspec_error(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.cts_to_erg, self.data.net_ctsspec_error, strict=False)
        ]
    ...
    def deconv_re_ergspec_error(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(
                self.re_cts_to_erg, self.data.net_re_ctsspec_error, strict=False
            )
        ]
