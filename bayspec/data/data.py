...
class Data:
    ...
    **Group docstring for aggregator properties.** Every list-valued
    ...
    def info(self):
        ...
        info = OrderedDict()
        info['Name'] = [key for key in self.data]
    ...
    def deconv_phtspec(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.fit_with.cts_to_pht, self.net_ctsspec, strict=True)
        ]
    ...
    def deconv_re_phtspec(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.fit_with.re_cts_to_pht, self.net_re_ctsspec, strict=True)
        ]
    ...
    def deconv_phtspec_error(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.fit_with.cts_to_pht, self.net_ctsspec_error, strict=True)
        ]
    ...
    def deconv_re_phtspec_error(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(
                self.fit_with.re_cts_to_pht, self.net_re_ctsspec_error, strict=True
            )
        ]
    ...
    def deconv_flxspec(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.fit_with.cts_to_flx, self.net_ctsspec, strict=True)
        ]
    ...
    def deconv_re_flxspec(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.fit_with.re_cts_to_flx, self.net_re_ctsspec, strict=True)
        ]
    ...
    def deconv_flxspec_error(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.fit_with.cts_to_flx, self.net_ctsspec_error, strict=True)
        ]
    ...
    def deconv_re_flxspec_error(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(
                self.fit_with.re_cts_to_flx, self.net_re_ctsspec_error, strict=True
            )
        ]
    ...
    def deconv_ergspec(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.fit_with.cts_to_erg, self.net_ctsspec, strict=True)
        ]
    ...
    def deconv_re_ergspec(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.fit_with.re_cts_to_erg, self.net_re_ctsspec, strict=True)
        ]
    ...
    def deconv_ergspec_error(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(self.fit_with.cts_to_erg, self.net_ctsspec_error, strict=True)
        ]
    ...
    def deconv_re_ergspec_error(self):
        ...
        return [
            factor * cts
            for (factor, cts) in zip(
                self.fit_with.re_cts_to_erg, self.net_re_ctsspec_error, strict=True
            )
        ]
    ...
    **Group docstring for computed properties.** One-line computed
    ...
    def _update_src(self):
        ...
        if isinstance(self._src, Source):
            ...
            if isinstance(self.src_file, BytesIO):
                ...
                raise ValueError('unsupported src file type')
    ...
    def _update_bkg(self):
        ...
        if self._bkg is None:
            ...
            self.bkg_ins = Background(
                self.src_ins_copy.counts,
            ...
            if isinstance(self._bkg, Background):
                ...
                if isinstance(self.bkg_file, BytesIO):
                    ...
                    raise ValueError('unsupported bkg file type')
        ...
        if self._rmf is None:
            ...
            if isinstance(self._rmf, Redistribution):
                ...
                if isinstance(self.rmf_file, BytesIO):
                    ...
                    raise ValueError('unsupported rmf file type')
        ...
        if self._arf is None:
            ...
            if isinstance(self._arf, Auxiliary):
                ...
                if isinstance(self.arf_file, BytesIO):
                    ...
                    raise ValueError('unsupported arf file type')
    ...
    def _update_rsp(self):
        ...
        if self._rsp is None:
            ...
            if self.rmf is not None and self.arf is not None:
                ...
                self.rsp_ins = None
                warnings.warn(f'response is missing for spectrum {self.src_name}', stacklevel=2)
            ...
            if isinstance(self._rsp, Response):
                ...
                if isinstance(self.rsp_file, BytesIO):
                    ...
                    raise ValueError('unsupported rsp file type')
    ...
    def completeness(self):
        ...
        return not (self.src_ins is None or self.rsp_ins is None)
    ...
    def _update_rebn(self):
        ...
        if self._rebn is not None and not isinstance(self._rebn, dict):
    ...
    def _update_time(self):
        ...
        if self._time is not None and not isinstance(self._time, (int, float)):
    ...
    def _update(self):
        ...
        if self.completeness:
            ...
            warnings.warn('data unit is uncomplete with missing src or rsp!', stacklevel=2)
    ...
    def cfg(self):
        ...
        for cid, cg in enumerate(self.config.values(), 1):
            cfg[str(cid)] = cg
    ...
    def par(self):
        ...
        for pid, pr in enumerate(self.params.values(), 1):
            par[str(pid)] = pr
    ...
    def rsp_drm(self):
        ...
        rsp_drm_cumsum = np.hstack(
            [
                np.zeros((self.rsp_ins.drm.shape[0], 1), dtype=float),
    ...
    def net_counts_upperlimit(self, cl=0.9):
        ...
        return (
            special.gammaincinv(
                N + 1, cl * special.gammaincc(N + 1, B) + special.gammainc(N + 1, B)
            )
            - B
        )
    ...
    def _notice(chbin, notc=None):
        ...
        for _, (low, upp) in enumerate(notc):
            flag_i = list(map(lambda lo, up: low <= lo and upp >= up, chbin[:, 0], chbin[:, 1]))
