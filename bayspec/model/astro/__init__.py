...
astromodel_dict = astromodels.list_functions()
...
add_model_types = {name: 'add' for name in add_model_classes}

mul_model_list = ['PhAbs', 'TbAbs', 'WAbs', 'EBLattenuation', 'ZDust']

mul_model_classes = dict()
for name in mul_model_list:
    with contextlib.suppress(AttributeError):
        mul_model_classes[name] = getattr(astromodels, name)
mul_model_types = {name: 'mul' for name in mul_model_classes}
...
math_model_types = {name: 'math' for name in math_model_classes}
...
def list_astro_models():
    ...
    return [f'AS_{name}' for name in model_classes]
...
for name, cls in model_classes.items():
                ...
                if pr.free:
                    ...
                    if pr.is_normalization and pl != 'NH':
                        ...
                        if max_value is None:
                            max_value = value * 1e5
                        self.params[f'log{pl}'] = Par(
                            np.log10(value), unif(np.log10(min_value), np.log10(max_value))
                        )
                    else:
    ...
    new_class = type(
        ...
        (Model,),
        {
            ...
            'func': func,
            '__doc__': f'astromodels ``{name}`` model bridged into bayspec.',
        },
    )
...
astro_models = {
    ...
    if isinstance(cls, type) and issubclass(cls, Model) and name[:2] == 'AS'
}
